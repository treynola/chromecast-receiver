...
(function () {
    ...
    /**
     * Z-Index management for dialogs
     */
    let topZIndex = 30000;
    
    window.bringDialogToFront = function (dialogElement) {
        if (!dialogElement) return;
        topZIndex++;
        dialogElement.style.zIndex = topZIndex.toString();
    };
    ...
    window.makeDraggable = function (element, handle) {
        ...
        // Also bring to front when clicking anywhere on the dialog
        element.addEventListener('mousedown', () => {
            if (window.bringDialogToFront) window.bringDialogToFront(element);
        }, true); // Use capture phase to trigger before other inputs
    };
