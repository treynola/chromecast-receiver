        ...
        container.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('fx-chain-slot-label')) {
                ...
                if (checkbox && checkbox.checked) {
                    ...
                    if (dialog) {
                        if (!dialog.open) {
                            if (window.bringDialogToFront) window.bringDialogToFront(dialog);
                            dialog.show();
                        } else {
                            if (window.bringDialogToFront) window.bringDialogToFront(dialog);
                        }
            ...
            if (target.classList.contains('effect-type-select')) {
                ...
                try {
                    ...
                    if (dialogContent) {
                        ...
                        if (window.bringDialogToFront) window.bringDialogToFront(track.elements.auditionDialog);
                        track.elements.auditionDialog.show();
    ...
    async function stopTrackRecording(trackId, btn) {
        ...
        try {
            ...
            if (blob && blob.size > 0) {
                ...
                if (typeof track.resetLoopPoints === 'function') track.resetLoopPoints();
            }
        } catch (err) {
