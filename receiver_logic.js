      ...
      (function () {
        ...
        window._useWebRTC = false; // [v13.9.472] APOR V2 Primary (LOCK)
        var configReceived = false;
        var targetRate = 48000;
        window._studioRate = 48000;
        window._hwRate = 48000;
        const VERSION_TAG = "v13.9.472-APORv2";
        const CUSTOM_NAMESPACE = "urn:x-cast:com.nowmultimedia.mxs004";

        // [v13.9.472] Dynamically build a valid 2-second silent WAV loop for TV OS media wake-lock
        function createSilentWavUrl() {
          ...
          // [v13.9.472] Write alternating 1 and -1 to render an inaudible dither signal (-90.3 dBFS)
        ...
        async function initAudio() {
          // [v13.9.472] WebRTC TRANSITION: Disable legacy PCM worklet if WebRTC is the goal
          ...
          // [v13.9.472] THROTTLE: Prevent tight-loop retries if init fails (e.g. 404 or SyntaxError)
          const now = Date.now();
          if (now - lastInitAttempt < 5000) return;
          lastInitAttempt = now;

          // [v13.9.472] HARDWARE LOCK: Never initialize until we have a verified sample rate from the Studio.
          if (!configReceived) {
          ...
          try {
            let workletUrl = `pcm-player-worklet-v13-9-472.js?cb=${Date.now()}`;
            ...
            // [v13.9.472] DYNAMIC RATE TRANSFORMATION
            ...
            workletNode.port.onmessage = (e) => {
              if (e.data.type === "DIAG") {
                ...
                // [v13.9.472] TRIPLE CHECK: Relay lock status to Studio every ~10s
                if (
        ...
        function relayLogToStudio(msg) {
          ...
          // [v13.9.472] Suppress DOM updates during active streaming to reduce TV CPU overhead
          ...
          let sent = false;
          // [v13.9.472] PREFER BINARY WS: Fastest and most reliable path
          if (binaryWS && binaryWS.readyState === WebSocket.OPEN) {
            try {
          ...
          // [v13.9.472] FALLBACK: Google Cast SDK Namespace
          ...
          // [v13.9.472] ULTIMATE FALLBACK: HTTP Beacon (Log Server)
          if (!sent && !isHighFreq) {
            logQueue.push(msg);
        ...
        const RENDER_THROTTLE_MS = 2000; // [v13.9.472] Throttle to 0.5 FPS — UI is decorative, audio is critical
        ...
        function connectBinaryBridge(ip, customPort, customToken) {
          if (
            ...
            // [v13.9.472] Already connected to this Studio IP. Ignore heartbeat redundancy.
            return;
          }
          // [v13.9.472] Guard against connecting while another connect is in progress
          ...
          binaryWS.onopen = async () => {
            ...
            // [v13.9.472] Reset reconnect backoff counter on success
            ...
            // [v13.9.472] HARDWARE PROBE: Detect actual device sample rate natively
            let hwRate = 48000;
            try {
          ...
          binaryWS.onmessage = (event) => {
            // [v13.9.472] PRIORITY: Binary audio data gets the fastest path — no parsing or async promise overhead
            if (event.data instanceof ArrayBuffer) {
              if (workletNode) {
                // [v13.9.472] APOR-WebRTC INTERLOCK
              ...
              // [v13.9.472] Fallback: TV browser ignored binaryType="arraybuffer"
              if (workletNode) {
                if (audioCtx && audioCtx.state === "suspended") resumeAudio();
              ...
              try {
                ...
                if (d.type === "STATE_UPDATE") {
                  ...
                  // [v13.9.472] Server confirmed handshake — init audio with negotiated config
          ...
          binaryWS.onclose = () => {
            ...
            // [v13.9.472] Reconnect with exponential backoff instead of full page reload
          ...
          binaryWS.onerror = (e) => {
            ...
            // [v13.9.472] Retry with full connection params (port + token preserved)
            if (wsConnectTimeout) clearTimeout(wsConnectTimeout);
            wsConnectTimeout = setTimeout(() => {
        ...
        window.onload = function () {
          ...
          if (typeof cast !== "undefined" && cast.framework) {
            try {
              ...
              // [v13.9.472] SENDER_CONNECTED/DISCONNECTED listeners
          ...
          const autoUnlock = setInterval(() => {
            ...
            // [v13.9.472] APOR-WebRTC FAILOVER
            ...
            // [v13.9.472] Non-Cast fallback only — keep HTML5 audio element alive
          ...
          // [v13.9.472] Global interaction listeners to catch TV remote keys and clicks for AudioContext unlock
