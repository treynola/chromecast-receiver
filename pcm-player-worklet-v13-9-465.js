...
/**
 * PCM Player AudioWorkletProcessor - TV-Side Resampling [v13-9-465]
 *
 * Fixes vs v13-9-465:
...
class PCMPlayerProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this._writePtr = 0;
    // [v13-9-465] FIX: Store read pointer as integer frame index + separate fraction
    ...
    this.port.onmessage = (e) => {
      try {
        ...
        // [v13-9-465] FIX: TEST_BEEP was called from index.html but never handled here.
  ...
  process(inputs, outputs) {
    try {
      ...
      // [v13-9-465] TEST_BEEP: Render a 1kHz sine wave to verify the audio path is working
      ...
      // [v13-9-465] FIX: Use Math.round() so available is always an integer,
      ...
      if (available < this._MIN_BUFFER) {
        this._stallCount++;
        this._isBuffering = true;
        // [v13-9-465] FIX: Reset smoothedError on stall so P-controller doesn't
      ...
      // [v13-9-465] STABILITY FIX: Use smoothedError for adjustment to filter out network jitter.
      ...
      // [v13-9-465] RESPONSIVENESS FIX: Increase smoothing weight to 0.1 (from 0.05)
      ...
      // [v13-9-465] FIX: Use integer frame index + separate fraction to eliminate
      ...
      // [v13-9-465] FIX: Track exact integer samples consumed for totalRead accuracy
      ...
      if (this._framesProcessed >= 144000) {
        ...
        // [v13-9-465] Also report deviation from baseRate for easier Studio diagnosis
