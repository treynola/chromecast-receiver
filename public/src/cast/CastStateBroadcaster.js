...
(function () {
    ...
    const BROADCAST_INTERVAL = 100; // 100ms = 10 updates/sec (UI mirror rate)

    // V2 Compat: Extract invoke at module level
    ...
    class CastStateBroadcaster {
        constructor() {
            ...
            if (!invoke) {
                console.warn("❌ CastStateBroadcaster: Tauri invoke NOT found.");
            }
        ...
        broadcastState() {
            try {
                ...
                // 1. Push to Google Cast (Hybrid Mode)
                if (invoke) {
                    invoke('push_chromecast_state', { data: stateJson }).catch((err) => {
                        if (this.frameCount % 50 === 0) console.error("❌ Failed to push chromecast state to Rust:", err);
                    });
                }

                // 2. Push to FCast (Custom Data Relay for Hybrid UI)
                // Using fcast_send_custom_data which mirrors payload to any FCast receiver
                if (invoke) {
                    invoke('fcast_send_custom_data', { 
                        data: stateJson, 
                        messageType: 'mxs_state' 
                    }).catch((err) => {
                        if (this.frameCount % 100 === 0) console.debug("📉 FCast push skipped (no connection):", err);
                    });
                }
        ...
        captureState() {
            ...
            if (!audioEngine || !tracksVis || tracksVis.length === 0 || !this._cachedElements) return null;

            // v13.7: Return plain state object — broadcastState() adds the wrapper.
            // Previously this returned {type:'STATE_UPDATE', state:{...}} which got
            // double-wrapped, causing receiver to fail silently on data.state.tracks.
            const state = {
                timestamp: Date.now(),
                tracks: [],
                transport: {},
            ...
            for (let i = 0; i < tracksVis.length; i++) {
                ...
                if (!trackView) continue;
                const audioTrack = trackView.trackAudio;
            ...
            state.transport = {
                isPlaying: audioEngine.isMasterRecording || false,
                bpm: (window.Tone && Tone.getTransport()) ? Tone.getTransport().bpm.value : 120,
                position: (window.Tone && Tone.getTransport()) ? Tone.getTransport().position : '0:0:0',
                loopTime: window.AppConfig?.LOOP_LENGTH || 4.0,
