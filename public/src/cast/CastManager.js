...
(function () {
    ...
    function addDeviceToList(device) {
        ...
        if (pLower === 'fcast') {
            ...
            protocolLabel = 'FCast';
            badgeHtml = '<span class="cast-recommended-badge">RECOMMENDED</span>';
            itemExtraStyle = 'border: 1px solid var(--green); box-shadow: 0 0 10px rgba(76, 175, 80, 0.2);';
        } else if (pLower === 'airplay') {
        ...
        item.style.cssText = `padding: 12px; margin: 8px 0; border: 1px solid #555; border-radius: 8px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; background: #2a2a2a; transition: all 0.2s ease-in-out; ${itemExtraStyle}`;
        item.innerHTML = `
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div>
                    <div style="font-weight: bold; font-size: 1.1em; display: flex; align-items: center; gap: 8px;">
                        ${device.name} 
                        <span style="font-size:0.7em; opacity:0.6;">(${protocolLabel})</span>
                        ${badgeHtml}
                    </div>
        ...
        item.onclick = async () => {
            ...
            try {
                ...
                if (pLower === 'chromecast' || pLower === 'chrome') {
                    // V118: Use the real WebSocket URL for signaling if available, fallback to legacy query string
                    const wsUrl = window.AudioCastManager?.currentWsUrl || `?audio_stream=${encodeURIComponent(audioUrl)}`;
                    console.log("🎬 Launching Hybrid with WS URL:", wsUrl);
                    await invokeCmd('chromecast_launch_hybrid', { deviceId: device.id, wsUrl: wsUrl });
                    console.log("✅ Custom App Loaded! Starting audio piping...");
                    // Note: The original startAudioPiping function does not take wsUrl as an argument.
                    // It's called later with 'webrtc' mode. This line is kept as per user's instruction,
                    // but it might not have the intended effect on the existing startAudioPiping.
                    // If the intent was to pass wsUrl to startAudioPiping, the function definition would need modification.
                    // For now, it's a call to a global function `startAudioPiping` with `wsUrl` as an argument.
                    if (typeof startAudioPiping === 'function') {
                        await startAudioPiping(wsUrl);
                    }
                ...
                if (err && err.toString && err.toString().includes('CAST_INIT_TIMEOUT')) {
                    errorMsg = 'TIMEOUT (macOS Permission?)';
                    alert("🛑 CAST_INIT_TIMEOUT: The MXS-004 receiver failed to launch.\n\nTroubleshooting:\n1. Ensure 'Local Network' permission is ON for MXS-004 in macOS System Settings.\n2. Reboot your Chromecast device.\n3. Check if your Mac and Chromecast are on the same subnet.");
                }

                item.style.background = '#5a2d2d';
                item.innerHTML = `<span><i class="fa-solid fa-warning"></i> ${errorMsg}</span>`;
            }
        };

        if (pLower === 'fcast') {
            deviceListEl.prepend(item);
        } else {
    ...
    window.stopAudioPiping = function () {
        if (window.castWorkletNode) {
            window.castWorkletNode.port.onmessage = null;
            window.castWorkletNode.disconnect();
            window.castWorkletNode = null;
        }
        audioPipeActive = false;
